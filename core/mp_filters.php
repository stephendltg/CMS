...
// filtre pour theme
add_filter('get_the_blog_theme', function( $theme ){ return sanitize_file_name($theme); } );

// filtre pour plugins
add_filter('get_the_blog_plugins', function( $plugins ){

    $plugins = explode(',', $plugins);
    if( is_sup($plugins,0) ){
        $plugins = array_map( function($plugin){ return sanitize_file_name($plugin) ;} , $plugins );
