...
/**
 * On réécrit l'url selon si mod rewrite actif ( apache ) ou pas get_permalink que pour les pages et tags
 * @return url
 */
function get_permalink( $slug ='' , $type ='page' ){
    ...
    if( $is_mod_rewrite ) {
        if( is_same($type , 'page') &&  is_page($slug) )
            $link = HOME .'/'. $slug;
        if( is_same($type, 'feed') && is_in($slug , array('rss','rss2','atom','rdf')) )
            $link = HOME .'/'. $slug .'.xml';
        ...
        if( is_same($type , 'page') &&  is_page($slug) )
            $link = HOME .'/index.php?p='.$slug;
        if( is_same($type, 'feed') && is_in($slug , array('rss','rss2','atom','rdf')) )
            $link = HOME .'/index.php?p='. $type .'.xml';
        if( is_same($type , 'page') &&  is_same($slug , 'sitemap') )
            $link = HOME .'/index.php?p=sitemap.xml';
        if( is_same($type , 'tag') &&  is_tag($slug) )
...
function is_feed(){
    ...
    return is_in( $query , array('rss.xml', 'atom.xml') );
}
